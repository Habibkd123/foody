...
export function middleware(request: NextRequest) {
  ...
  let userRole: string | undefined = request.cookies.get("user-role")?.value;
  if (!userRole) {
    const userCookie = request.cookies.get("user")?.value;
    if (userCookie) {
      try {
  ...
  // ✅ Allow SEO/system files always
  if (path === "/robots.txt" || path === "/sitemap.xml") {
    return NextResponse.next();
  }

  // ✅ Skip static files, APIs, assets
  if (
    path.startsWith("/_next/") ||
    path.startsWith("/api/") ||
    path.startsWith("/uploads/") ||
  ...
  // 🔁 If logged-in user visits login/auth → redirect by role
  if (
    isPublicRoute &&
    token &&
    userRole &&
  ...
  // 🔐 Not public → must be logged in
  if (!token) {
    return NextResponse.redirect(new URL("/login", request.url));
  }

  // 🎭 Role-based route access (ONLY for logged-in users)
