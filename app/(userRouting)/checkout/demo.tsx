...
//       price: item.price,
//     }));
//     // items, orderId, userId, total, method 
//     const res = await fetch("/api/create-order", {
//       method: "POST",
//       headers: { "Content-Type": "application/json" },
//       body: JSON.stringify({ items: products, orderId: "YOUR_ORDER_ID_HERE",total:totalAmount,method:"upi_debit",userId:user._id }),
//     });
...
//                 <button onClick={handleStripeCheckout} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-3 px-6 rounded-lg transition-colors duration-200">
//                   Complete Payment
//                 </button>
...
const cryptoOptions = [
  ...
  { name: "USDC", symbol: "USDC", logo: "$" }
];

const bnplOptions = [
  { name: "Klarna", logo: "K", color: "bg-pink-500" },
  { name: "Afterpay", logo: "A", color: "bg-green-500" },
  { name: "Affirm", logo: "A", color: "bg-blue-500" }
];
          ...
          <div className="w-full lg:w-2/3 space-y-6">
              ...
              <div className="p-6 bg-white rounded-xl border border-gray-100 shadow-sm">
                <div className="flex flex-wrap gap-4">
                  {DigitalWallets.map((wallet) => (
                    <div
                      key={wallet.title}
                      onClick={() => setSelectedDigitalWallets(wallet)}
                      className={`cursor-pointer w-40 p-4 rounded-lg border transition ${
                        selectedDigitalWallets?.title === wallet.title
                          ? "border-orange-500 bg-orange-50"
                          : "border-gray-900 bg-white hover:border-gray-300"
                      }`}
                    >
                      <div className="flex flex-col items-center">
                        ...
                        <p className={`text-xs font-medium flex items-center mt-1 ${
                          wallet.isLinked ? "text-green-600" : "text-orange-500"
                        }`}>
                          <Check size={14} className="mr-1" />
                          {wallet.isLinked ? "LINKED" : "NOT LINKED"}
                        </p>
                ...
                {selectedDigitalWallets && (
                  <div className="mt-6 border-t pt-4">
                    ...
                    />
                    <div className="flex items-center mb-4">
                      <select className="px-3 py-2 border border-gray-300 rounded-l-md bg-white text-gray-700" disabled>
                        <option value="+91">+91</option>
                      </select>
                      <Input
                        type="tel"
                    ...
                    <button
                      onClick={() => handleStripeCheckout('link')}
                      className={`w-full py-2 rounded-md text-white text-sm ${
                        isPhoneValid ? "bg-green-600 hover:bg-green-700" : "bg-gray-300 cursor-not-allowed"
                      }`}
                      disabled={!isPhoneValid}
                    >
                      Continue with Link
            ...
            <PaymentSection id="cards" disabled={false} scrolled={false} title="Credit or Debit Cards" icon={<CreditCard size={20} />} isActive={expanded === "cards"}>
              <div className="space-y-6">
                ...
                <div className="grid gap-4">
                  <input type="text" placeholder="Name on Card" className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" />
                  <input type="text" placeholder="Card Number" className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" />
                  <div className="grid grid-cols-2 gap-4">
                    <input type="text" placeholder="MM/YY" className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" />
                    <input type="text" placeholder="CVV" className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" />
                  </div>
                  <input type="text" placeholder="Nickname for card (Optional)" className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" />
                </div> */}
                {/* <button 
            ...
            {/* <PaymentSection id="netbanking" disabled={false} scrolled={true} title="Net Banking" icon={<Building size={20} />} isActive={expanded === "netbanking"}>
              <div className="space-y-6">
                  ...
                  {banks.map((bank) => (
                    <div
                      ...
                      className={`relative p-4 border-2 rounded-lg cursor-pointer transition-all duration-200 hover:shadow-md ${
                        selectedBank === bank.name ? "border-emerald-500 bg-emerald-50" : "border-gray-200 hover:border-gray-300"
                      }`}
                    >
                      <div className={`w-10 h-10 ${bank.color} rounded-lg mx-auto mb-2 flex items-center justify-center text-white text-lg`}>
                        {bank.logo}
                ...
                <button 
                  onClick={() => handleStripeCheckout('bancontact')}
