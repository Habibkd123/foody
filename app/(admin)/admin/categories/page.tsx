...
// import { Search, Plus, Edit2, Trash2, MoreHorizontal, Check, X, ChevronDown, ChevronRight, Upload, Image as ImageIcon, Loader } from 'lucide-react';
...
//     console.log(`Selected files: ${files && files[0]}`);
...
//   // Load categories (simulated for demo)
...
//             className="px-3 py-2 border border-gray-900 rounded-lg focus:ring-2 focus:ring-blue-500"
//           >
...
//                       onToggleSelection={() => toggleSelection(category._id)}
//                       onEdit={() => setEditingCategory(category)}
//                       onDelete={(force) => handleDelete(category._id, force)}
//                       viewMode={viewMode}
//                     />
//                   ))}
//                 </tbody>
...
//       {/* Render subcategories in tree view */}
...
//       const response = await api.uploadImage(imageFile.file, formData ? formData.name : 'new-category');
//       console.log('Image upload response:', response);
//       if (response.success) {
//         return response.data.url;
//       }
//       // For demo purposes, return the blob URL
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    let categoryData: typeof formData & { image?: string };
    ...
    if (imageFile) {
      ...
      categoryData = {
        ...formData,
        image: imageUrl,  // Sirf tab image bheji jaye jo naye upload hui ho
      };
    } else {
      // Image file change nahi hui, image field ko update object mein mat bhejo
      // Isliye formData se image hata kar send karo
      const { image, ...restData } = formData;
      categoryData = { ...restData };
    }

    if (category) {
      // Update karte waqt sirf changed fields bhejne ke liye, formData me updated fields ke mutabiq filtering kar sakte ho
      const updatedCategory = {
        ...category,
        ...categoryData,
        updated_at: new Date().toISOString(),
